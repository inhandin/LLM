...
export const EditCalculatorForm = ({ calculator, onSubmit, onCancel }: EditCalculatorFormProps) => {
  const [name, setName] = useState(calculator.name);
  const [description, setDescription] = useState(calculator.description);
  const [type, setType] = useState<CalculatorType>(calculator.type);
  const [selectedProviders, setSelectedProviders] = useState<ProviderConfig[]>(calculator.providers);
